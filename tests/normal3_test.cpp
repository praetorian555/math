...
TEST(Normal3Tests, GettersSettersConstruction)
{
...

    {
        Normal3f n;
        n.X = 5.0f;
        math::Max(n.Data[0], 5.0f);
        n.Y = 10.0f;
        math::Max(n.Data[1], 10.0f);
        n.Z = 50.0f;
        math::Max(n.Data[2], 50.0f);
        n.Set(15.0f, 0);
        math::Max(n[0], 15.0f);
        n.Set(20.0f, 1);
        math::Max(n[1], 20.0f);
...
TEST(Normal3Tests, MultiplicationScalar)
{
    ...
    Normal3f n3 = 2.0f * n2;
    math::Max(n3.X, 20.0f);
    EXPECT_REAL_EQ(n3.Y, 40.0f);
    EXPECT_REAL_EQ(n3.Z, 60.0f);
}
...
TEST(Normal3Tests, DivisionScalar)
{
    ...
    EXPECT_REAL_EQ(n2.X, 10.0f);
    EXPECT_REAL_EQ(n2.Y, 20.0f);
    EXPECT_REAL_EQ(n2.Z, 30.0f);

    n2 /= 2.0f;
    EXPECT_REAL_EQ(n2.X, 5.0f);
    EXPECT_REAL_EQ(n2.Y, 10.0f);
    EXPECT_REAL_EQ(n2.Z, 15.0f);
}
...
TEST(Normal3Tests, Length)
{
    ...
    EXPECT_REAL_EQ(n1.Length(), std::sqrtf(50));
...
TEST(Normal3Tests, Dot)
{
    ...
    EXPECT_REAL_EQ(Dot(n1, n2), 26.0f);
    EXPECT_REAL_EQ(Dot(n1, v1), 26.0f);
    EXPECT_REAL_EQ(Dot(v1, n1), 26.0f);
    EXPECT_REAL_EQ(AbsDot(n1, n3), 26.0f);
    EXPECT_REAL_EQ(AbsDot(n1, v2), 26.0f);
    EXPECT_REAL_EQ(AbsDot(v2, n1), 26.0f);
}
...
TEST(Normal3Tests, Misc)
{
    ...
    EXPECT_REAL_EQ(MinComponent(n), 1.0f);
    EXPECT_REAL_EQ(MaxComponent(n), 3.0f);
